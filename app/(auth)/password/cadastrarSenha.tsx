...
import { ResetarSenha } from '@/app/Services/RecuperacaoDeSenhaServices';

export default function CadastrarSenhaRecuperacao() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await ResetarSenha({
        codToken: codigo as string,
        senhaBen: password,
        confSenhaBen: confirmPassword
      });

      if(response.status === 200 || response.status == 201) {
          Alert.alert('Resetar Senha', response.data.mensagem);
          router.replace({
              pathname: '/(auth)/login',
    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={styles.header}>
        <Image 
          ...
          resizeMode='contain'
        />
        <TextPadrao text="Crie sua nova senha." />
      </View>
